...
int nty_event_del(int epfd, struct ntyevent *ev) {
	...
	ep_ev.data.ptr = ev;
	ev->status = 0;
	epoll_ctl(epfd, EPOLL_CTL_DEL, ev->fd, &ep_ev);//加入，删除 修改状态位

	return 0;
}

int recv_cb(int fd, int events, void *arg) {//通过arg获取注册的ev事件
...
int ntyreactor_addlistener(struct ntyreactor *reactor, int sockfd, NCALLBACK *acceptor) {
	...
	nty_event_set(&reactor->events[sockfd], sockfd, acceptor, reactor);//通过fd找到events[sockfd]
...
int ntyreactor_run(struct ntyreactor *reactor) {
	...
	struct epoll_event events[MAX_EPOLL_EVENTS+1];//epoll_wait返回出来的事件集合
