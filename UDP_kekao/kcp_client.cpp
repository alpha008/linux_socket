...
int main(int argc, char* argv[])
{
	...
	while (1) {
		...
		for (; current >= slap; slap += 20) {
			((IUINT32*)buffer)[0] = index++;
			((IUINT32*)buffer)[1] = current;
			ikcp_send(kcp1, buffer, 8);// 发送上层协议包 // 此处发送数据
		}
		// kcp1收到kcp2的回射数据
		while (1) {
			hr = ikcp_recv(kcp1, buffer, 10);// 接收对端发送来的数据进行解析
			if (hr < 0) break;// 没有收到包就退出
			IUINT32 sn = *(IUINT32*)(buffer + 0);// 序列号
			IUINT32 ts = *(IUINT32*)(buffer + 4);// 时间序列
			IUINT32 rtt = current - ts;	// round trip time		
			if (sn != next) {		
				printf("ERROR sn %d<->%d\n", (int)count, (int)next);// 如果收到的包不连续
				return NULL;
			}
			next++;
			sumrtt += rtt;
