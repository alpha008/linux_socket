...
int ikcp_recv(ikcpcb *kcp, char *buffer, int len)
{
	...
	if (peeksize > len)     // 可读数据大于 用户传入的长度，每次读取需要一次性读取完毕
		return -3;
...
//---------------------------------------------------------------------
/*
把用户发送的数据根据MSS(max segment size)分片成KCP的数据包格式，插入待发送队列中。当用户的数据超过一个MSS(最大分片大小)
的时候，会对发送的数据进行分片处理。通过frg进行排序区分，frg即message中的segment分片ID，在message中的索引，由大到小，
0表示最后一个分片。分成4片时，frg为3,2,1,0。
如用户发送2900字节的数据，MSS为1400byte。因此，该函数会把1900byte的用户数据分成两个包，一个数据大小为1400，头frg设置为2，
len设置为1400；第二个包，头frg设置为1，len设置为1400; 第三个包, 头frg设置为0，len设置为100。
切好KCP包之后，放入到名为snd_queue的待发送队列中。
*/

/*分片方式共有两种。
(1) 流模式情况下，检测每个发送队列里的分片是否达到最大MSS，如果没有达到就会用新的数据填充分片。
接收端会把多片发送的数据重组为一个完整的KCP包。
...
void ikcp_flush(ikcpcb *kcp)
{
	...
	if (lost) {
		...
		if (kcp->ssthresh < IKCP_THRESH_MIN)
			kcp->ssthresh = IKCP_THRESH_MIN;
		kcp->cwnd = 1;  //丢失则阈值减半, cwd 窗口保留为 1 
		kcp->incr = kcp->mss;
	}
