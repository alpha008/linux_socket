...
int main(int argc, char *argv[]) {
	...
	while (1) {
		rset = rfds;     //，下次循环又回来了，将用户态操作的放置到内核态赋值
		...
		if (FD_ISSET(sockfd, &rset)) { //accept
			...
			if (clientfd > max_fd) max_fd = clientfd;//在accept时会重新得到fd，更新max_fd的值
		...
		for (i = sockfd + 1;i <= max_fd;i ++) {
			if (fds[i].revents & (POLLIN|POLLERR)) {
				...
				if (ret < 0) {
					...
					//close(i);
					fds[i].fd = -1;
				} else if (ret == 0) {// close的时候会返回0，缓冲区没了
					printf(" disconnect %d\n", i);
	...
	while (1) {
		...
		int nready = epoll_wait(epoll_fd, events, EPOLL_SIZE, -1);//events 这里是内核传出来的就绪的IO
		if (nready == -1) {
			printf("epoll_wait\n");
			break;
		}
		...
		for (i = 0;i < nready;i ++) {
			if (events[i].data.fd == sockfd) {
				...
				if (ret < 0) {
					...
					ev.events = EPOLLIN | EPOLLET;  //边沿触发 +  从无到有触发        
					// LT，当有了数据后就一直触发
					ev.data.fd = clientfd;                //删除fd
