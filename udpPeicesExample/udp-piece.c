...
udp_piece_t* udp_piece_init( int buf_size )
{
	...
	if ( !udp_piece )
		return(NULL);
	memset( udp_piece, 0, sizeof(udp_piece_t) );// 分配并初始化管理结构体

	udp_piece->circular_buffer = circular_buffer_init( buf_size );
	if ( !udp_piece->circular_buffer )// 初始化环形缓冲区
		return(NULL);
	return(udp_piece);
}
...
uint8_t *udp_piece_get( udp_piece_t *udp_piece, int index, int *got_piece_size )
{
	...
	*got_piece_size = 0;
	...
	// 分片大小    // 是不是最后一个分片
	if(((udp_piece->total_pieces - 1) == index)&& (udp_piece->left > 0){
		piece_size = udp_piece->left;// 最后一个分片大小
	}
	else{	
		piece_size = PIECE_FIX_SIZE; // 固定大小分片
	}
	// 初始化为0
	memset( udp_piece->piece_buf, 0, sizeof(udp_piece->piece_buf) );
	// 填充分片帧头 同步字
	udp_piece->piece_buf[HEAD_POS_SYNC_WORD] = 0xAF;  // 同步字
	udp_piece->piece_buf[HEAD_POS_SYNC_WORD + 1] = 0xAE; // 同步字
	// 所有分片数据的大小 + 取低8位
	...
	*got_piece_size = piece_size + HEAD_SIZE;// 数据片大小 + 头大小
...
int udp_piece_merge_ex( udp_piece_t *udp_piece, void *buf, int size )
{
	...
	/* 如果剩余的数据长度仍大于于 */
	while ( circular_buffer_size( udp_piece->circular_buffer ) > HEAD_SIZE ){/* 当前分片的数据长度（不含帧头） */
		...
		if ( circular_buffer_size( udp_piece->circular_buffer ) >= (HEAD_SIZE + data_len) ){
			...
			// 读取分片数据
			bytes_to_read = circular_buffer_read( udp_piece->circular_buffer,&udp_piece->recv_buf[PIECE_FIX_SIZE * p_index],data_len );
